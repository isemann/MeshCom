...
static void drawTextMessageFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    ...
    display->setTextAlignment(TEXT_ALIGN_LEFT);
    display->setFont(FONT_SMALL); //RKE FONT_MEDIUM have been previous
    String sender = (node && node->has_user) ? node->user.long_name : "???"; //RKE show long name instead short name -> more afu alike
